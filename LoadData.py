...
def main():
    ...
    # dayv2pub
    colNames="HOUSEID,PERSONID,FRSTHM,OUTOFTWN,ONTD_P1,ONTD_P2,ONTD_P3,ONTD_P4,ONTD_P5,ONTD_P6,ONTD_P7,ONTD_P8,ONTD_P9,ONTD_P10,ONTD_P11,ONTD_P12,ONTD_P13,ONTD_P14,ONTD_P15,TDCASEID,HH_HISP,HH_RACE,DRIVER,R_SEX,WORKER,TRIPPURP,AWAYHOME,DROP_PRK,DRVR_FLG,EDUC,ENDTIME,HH_ONTD,HHMEMDRV,HHRESP,INTSTATE,MSASIZE,NONHHCNT,NUMONTRP,PAYTOLL,PRMACT,PROXY,PSGR_FLG,R_AGE,STRTTIME,TRACC1,TRACC2,TRACC3,TRACC4,TRACC5,TRACCTM,TRAVDAY,TREGR1,TREGR2,TREGR3,TREGR4,TREGR5,TREGRTM,TRPACCMP,TRPHHACC,TRPHHVEH,TRPTRANS,TRVL_MIN,TRVLCMIN,TRWAITTM,USEINTST,USEPUBTR,VEHID,WHODROVE,WHYFROM,WHYTO,WHYTRP1S,DWELTIME,WHYTRP90,TDTRPNUM,TDWKND,TDAYDATE,TRPMILES,WTTRDFIN,VMT_MILE,PUBTRANS,HTPPOPDN,GASPRICE,VEHTYPE,HHC_MSA".split(',')
    ...
    EIA_files_indexes = [[x for x in range(1,4)] for __ in range(3)]

            
    for i, myfile in enumerate(NHTS_files):
       #loadTables(myfile, cur, 'XX')
       loadTables2(myfile, cur, conn, NHTS_files_indexes[i], nullValues)

    for i, myfile in enumerate(EIA_files):
       #loadTables(myfile, cur, 'Not Available')
       loadTables2(myfile, cur, conn, EIA_files_indexes[i], nullValues)
...
def createTables(cur):
    #cur.execute('CREATE TABLE EIA_MkWh_2015(MSN CHAR(8), Date INT, Value DOUBLE PRECISION, Column_Order INT, DESCRIPTION VARCHAR(100), UNIT VARCHAR(100));')

    #cur.execute('CREATE TABLE EIA_CO2_Electricity_2015(MSN CHAR(8), Date INT, Value DOUBLE PRECISION, Column_Order INT, DESCRIPTION VARCHAR(100), UNIT VARCHAR(100));')

    #cur.execute('CREATE TABLE EIA_CO2_Transportation_2015(MSN CHAR(8), Date INT, Value DOUBLE PRECISION, Column_Order INT, DESCRIPTION VARCHAR(100), UNIT VARCHAR(100));')

    cur.execute('CREATE TABLE EIA_MkWh_2015(Date INT, Value DOUBLE PRECISION, Column_Order INT);')

    cur.execute('CREATE TABLE EIA_CO2_Electricity_2015(Date INT, Value DOUBLE PRECISION, Column_Order INT);')

    cur.execute('CREATE TABLE EIA_CO2_Transportation_2015(Date INT, Value DOUBLE PRECISION, Column_Order INT);')
    ...
    cur.execute('CREATE TABLE DAYV2PUB(HOUSEID int, PERSONID int, FRSTHM int, OUTOFTWN int, ONTD_P1 int, ONTD_P2 int, ONTD_P3 int, ONTD_P4 int, ONTD_P5 int, ONTD_P6 int, ONTD_P7 int, ONTD_P8 int, ONTD_P9 int, ONTD_P10 int, ONTD_P11 int, ONTD_P12 int, ONTD_P13 int, ONTD_P14 int, ONTD_P15 int, TDCASEID bigint, HH_HISP int, HH_RACE int, DRIVER int, R_SEX int, WORKER int, TRIPPURP varchar(8), AWAYHOME int, DROP_PRK int, DRVR_FLG int, EDUC int, ENDTIME int, HH_ONTD int, HHMEMDRV int, HHRESP int, INTSTATE int, MSASIZE int, NONHHCNT int, NUMONTRP int, PAYTOLL int, PRMACT int, PROXY int, PSGR_FLG int, R_AGE int, STRTTIME int, TRACC1 int, TRACC2 int, TRACC3 int, TRACC4 int, TRACC5 int, TRACCTM int, TRAVDAY int, TREGR1 int, TREGR2 int, TREGR3 int, TREGR4 int, TREGR5 int, TREGRTM int, TRPACCMP int, TRPHHACC int, TRPHHVEH int, TRPTRANS int, TRVL_MIN int, TRVLCMIN int, TRWAITTM int, USEINTST int, USEPUBTR int, VEHID int, WHODROVE int, WHYFROM int, WHYTO int, WHYTRP1S int, DWELTIME int, WHYTRP90 int, TDTRPNUM int, TDWKND int, TDAYDATE int, TRPMILES double precision, WTTRDFIN double precision, VMT_MILE double precision, PUBTRANS int, HTPPOPDN int, GASPRICE double precision, VEHTYPE int, HHC_MSA varchar(4));')

    cur.execute('CREATE TABLE VEHV2PUB(HOUSEID int, WTHHFIN double precision, VEHID int, HYBRID int, MAKECODE int, MODLCODE int, MSASIZE int, OD_READ int, TRAVDAY int, VEHCOMM int, VEHOWNMO double precision, VEHYEAR int, WHOMAIN int, VEHAGE int, PERSONID int, HH_HISP int, HH_RACE int, ANNMILES double precision, HTPPOPDN int, BEST_FLG int, BESTMILE double precision, BEST_EDT int, BEST_OUT int, FUELTYPE int, GSYRGAL int, GSCOST double precision, GSTOTCST int, EPATMPG double precision, EPATMPGF int, EIADMPG double precision, VEHTYPE int, HHC_MSA varchar(4));')

    cur.execute('CREATE TABLE PERV2PUB(HOUSEID int, PERSONID int, VARSTRAT int, WTPERFIN double precision, SFWGT double precision, HH_HISP int, HH_RACE int, CNTTDTR int, BORNINUS int, CARRODE int, CONDNIGH int, CONDPUB int, CONDRIDE int, CONDRIVE int, CONDSPEC int, CONDTAX int, CONDTRAV int, DELIVER int, DIARY int, DISTTOSC int, DRIVER int, DTACDT int, DTCONJ int, DTCOST int, DTRAGE int, DTRAN int, DTWALK int, EDUC int, EVERDROV int, FLEXTIME int, FMSCSIZE int, FRSTHM int, FXDWKPL int, GCDWORK numeric, GRADE int, GT1JBLWK int, HHRESP int, ISSUE int, OCCAT int, LSTTRDAY int, MCUSED int, MEDCOND int, MEDCOND6 int, MOROFTEN int, NBIKETRP int, NWALKTRP int, OUTCNTRY int, OUTOFTWN int, PAYPROF int, PRMACT int, PROXY int, PTUSED int, PURCHASE int, R_AGE int, R_RELAT int, R_SEX int, SAMEPLC int, SCHCARE int, SCHCRIM int, SCHDIST int, SCHSPD int, SCHTRAF int, SCHTRN1 int, SCHTRN2 int, SCHTYP int, SCHWTHR int, SELF_EMP int, TIMETOSC int, TIMETOWK int, TOSCSIZE int, TRAVDAY int, USEINTST int, USEPUBTR int, WEBUSE int, WKFMHMXX int, WKFTPT int, WKRMHM int, WKSTFIPS int, WORKER int, WRKTIME varchar(7), WRKTRANS int, YEARMILE int, YRMLCAP int, YRTOUS int, DISTTOWK double precision, HTPPOPDN int, HHC_MSA varchar(4));')
...
def loadTables2(filename, cur, conn, valuesIndex, nullValues):
    ...
    #with open('/home/cjnitta/ecs165a/subset/%s' %(filename)) as f:
        counter = 0
